...
<div id="gnb" class="hd_div">
    <div class="tnb">
        <?php if ($is_member) {  ?>
        <span class="niname"><?php echo get_member_profile_img($member['mb_id']); ?> <strong><?php echo $member['mb_id'] ? $member['mb_name'] : '비회원'; ?></strong> 님 </span>
    ...
    <div id="cate_01">
        ...
        ?>
            <li class="cate_li">
                <a href="<?php echo $mshop_ca_href.$mshop_ca_row1['ca_id']; ?>" class="cate1_a"><?php echo get_text($mshop_ca_row1['ca_name']); ?></a>
                <?php
                $mshop_ca_res2 = sql_query(get_mshop_category($mshop_ca_row1['ca_id'], 4));
                if(sql_num_rows($mshop_ca_res2))
                    echo '<button class="sub_ct_toggle ct_op"><span class="sound_only">'.get_text($mshop_ca_row1['ca_name']).'하위분류</span><i class="fa fa-sort-down"></i></button>'.PHP_EOL;
                ...
                ?>
                    <li class="sub_cate1_li">
                        ...
                        if(sql_num_rows($mshop_ca_res3))
                            echo '<button type="button" class="sub_ct_toggle ct_op"><span class="sound_only">'.get_text($mshop_ca_row2['ca_name']).' 하위분류 </span><i class="fa fa-sort-down"></i></button>'.PHP_EOL;
                        ...
                        ?>
                            <li>
                                ...
                                if(sql_num_rows($mshop_ca_res4))
                                    echo '<button type="button" class="sub_ct_toggle ct_op"><span class="sound_only">'.get_text($mshop_ca_row3['ca_name']).' 하위분류 </span><i class="fa fa-sort-down"></i></button>'.PHP_EOL;
                                ...
                                ?>
                                    <li>
                                        ...
                                        if(sql_num_rows($mshop_ca_res5))
                                            echo '<button type="button" class="sub_ct_toggle ct_op"><span class="sound_only">'.get_text($mshop_ca_row4['ca_name']).' 하위분류 </span><i class="fa fa-sort-down"></i></button>'.PHP_EOL;
        ...
        if($i > 0)
            echo '<li class="cate_li"><a href="'.G5_URL.'" class="cate1_a"> 커뮤니티</a></li></ul>'.PHP_EOL;
        else
